...
- **[IAM Baseline Matrix & Guardrails](contracts/iam-baseline-matrix.md)**: Standardized IAM roles, OIDC pipeline federations, Service Control Policies (SCPs), and Resource Control Policies (RCPs with service exceptions).
- **[Network CIDR & IPAM Hierarchy](contracts/network-cidr-ipam-allocations.md)**: RFC 1918 supernet allocations, IPv6 GUA allocations, TGW route table matrix, and AZ subnet distribution.
...
- **[Terragrunt Root Configuration](iac-catalogs/terragrunt-root.hcl)**: Dynamic provider generation, KMS CMK-encrypted S3 remote state backend, DynamoDB lock configurations, and mandatory FinOps default tags.
...
flowchart TB
    subgraph Global_Edge_Perimeter["Domain 09: Global Edge Perimeter & Route 53 DNS"]
        R53_ARC["Route 53 ARC Routing Controls (5-Region Quorum + Zonal Autoshift)"]
        CloudFront_Edge["Amazon CloudFront CDN (Origin Shield & OAC)"]
        WAF_Global["AWS WAF (Bot Control / Rate Limiting / Staged Count-to-Block)"]
    ...
    subgraph AWS_Hub_and_Governance["Core Hub & Central Governance Accounts"]
        ...
        subgraph Domain_01_02["Domain 01 & 02: Network Hub & Hybrid Ingress"]
            TGW_Core["AWS Transit Gateway (Multi-AZ /27 Subnets)"]
            NFW_Cluster["AWS Network Firewall (Stateful DPI)"]
            DXGW_CloudWAN["Direct Connect Gateway & AWS Cloud WAN (BGP Communities)"]
            R53_Profiles_Hub["Route 53 Profiles (RAM Shared Org-wide)"]
        end
        ...
        subgraph Domain_03_04_05["Domain 03, 04 & 05: SecOps, Observability & FinOps"]
            ...
            KMS_MultiRegion_Vault["KMS Multi-Region CMKs (Synchronized Policies) & Secrets Manager"]
            CW_OAM_OpenSearch["CloudWatch OAM Sink & Amazon OpenSearch Multi-AZ"]
            S3_Log_Archive["S3 Log Vault (Object Lock Compliance Mode & Glacier Deep Archive)"]
            CUR_FinOps_Lake["CUR 2.0 Parquet & Athena Partition Projection"]
        end
    ...
    subgraph Primary_Region_Workloads["Primary Region (us-east-1): Production Workloads"]
        ...
        subgraph Domain_07["Domain 07: Compute Platforms & Service Mesh"]
            EKS_Cluster_Prod["Amazon EKS (Karpenter Graviton / Bottlerocket / Disruption Budgets)"]
            VPC_Lattice_Mesh["Amazon VPC Lattice (Gateway API Controller / Zero-Trust SigV4 Mesh)"]
        end
        ...
        subgraph Domain_06["Domain 06: Data Persistence & Streaming"]
            ...
            MSK_Kafka_Streams["Amazon MSK Kafka (PrivateLink + Tiered Storage) & Iceberg Lakehouse"]
        end

        subgraph Domain_08["Domain 08: Asynchronous Orchestration"]
            EB_Central_Bus["Amazon EventBridge Central Bus (Schema Registry & Replay)"]
            StepFunctions_Saga["AWS Step Functions (Distributed Saga with MaxConcurrency)"]
            SQS_SNS_Fanout["Amazon SQS FIFO & SNS Data Protection & Pipes DLQ Redrive"]
        end

        subgraph Domain_10["Domain 10: AI/ML Inference & Guardrails"]
            Bedrock_Engine["Amazon Bedrock (Cross-Region Inference Profiles: Claude 3.5 / Titan)"]
            Bedrock_Guardrails["Bedrock Guardrails (Mandatory bedrock:GuardrailIdentifier Condition)"]
            AOSS_Vector_RAG["OpenSearch Serverless (Tenant-Filtered Vector RAG Index)"]
        end
    ...
    subgraph Secondary_DR_Region["Secondary DR Region (us-west-2): Domain 11 Resilience"]
        ...
        RDS_Proxy_Standby["Standby RDS Proxy (Pre-provisioned)"]
    ...
    CloudFront_Edge -->|SigV4 Signed Ingress + PL Cloaking| EKS_Cluster_Prod
    ...
    Aurora_Secondary --> RDS_Proxy_Standby
...
### Playbook 1: Automated Regional Catastrophic Disaster Recovery Failover (Route 53 ARC + Step Functions Orchestration)
...
2. **Execution Steps**:
   - **Step 1**: SRE on-call executes Route 53 ARC CLI command or automated Step Function trying the 5 regional cluster endpoints to atomically flip Routing Control from `us-east-1` (0) to `us-west-2` (1):
     ```bash
     ...
     ```
   - **Step 2**: Automated Step Function atomically revokes primary region Aurora DB security group ingress to prevent split-brain write conditions.
   - **Step 3**: Verify replication lag is drained ($< 1\text{ s}$) and promote the secondary Aurora PostgreSQL cluster to standalone writer:
     ```bash
     aws rds failover-global-cluster \
       --global-cluster-identifier enterprise-aurora-global \
       --target-db-cluster-identifier enterprise-aurora-secondary-cluster
     ```
   - **Step 4**: Pre-provisioned standby RDS Proxy in `us-west-2` automatically routes traffic to the newly promoted writer; Karpenter scales EKS pods based on target traffic demand.
